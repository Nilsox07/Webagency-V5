...
1 — HTTP-AUSLIEFERUNG
Gemessen: /sitemap.xml, /robots.txt und /llms.txt tragen Content-Type text/html, dazu X-Content-Type-Options nosniff. Eine als HTML ausgelieferte Sitemap liest Google nicht als Sitemap. Ursache ist app/Antwort.php Zeile 27: der PHP-Operator + behält den linken Schlüssel, jede übergebene Kopfzeile verliert. Der Klassenkommentar von datei() beschreibt diese Falle bereits und begründet eine eigene Fabrik — Wurzeldateien benutzt sie nicht.
Ausserdem setzt jede öffentliche Antwort ein PHPSESSID-Cookie und Cache-Control no-store, no-cache, must-revalidate, auch robots.txt. Ursache: public/index.php startet die Sitzung vor der Routenentscheidung.
...
composer.lock zeigt, dass setasign/fpdf ext-gd verlangt; composer.json deklariert es nicht und LIVEGANG.md nennt es nicht als Voraussetzung. Ein frisches composer install bricht auf jedem Server ohne GD ab, und drei Bildtests scheitern daran. Trag ext-gd ein, aktualisiere die Lockdatei so, dass composer validate --strict sauber läuft, und ergänze die Livegangliste.
...
a) Bedarfsscheck Thema 5, Feld 5.2 Logostatus mit vier Optionen: Logo vorhanden als Datei · Logo vorhanden, aber nur gedruckt oder als Foto · Kein Logo · Bin unsicher. Zeile 219. Der Logostatus ändert den Leistungsweg vor dem Festpreis.
...
c) /foerderung enthält null Links. app/services/Foerderprogramme.php hat 0 URLs, und die Seite behauptet sichtbar, alle Angaben seien an der Quelle geprüft. FOERDERUNG_KONZEPT.md legt als Architektur verweisen statt kopieren fest — die Links sind das Konzept, weil bei der Recherche sechs von sechzehn Sekundärangaben falsch waren. Trag die 16 offiziellen Adressen ein. Findest du eine nicht, nimm die Behauptung der Quellenprüfung von der Seite, statt sie stehen zu lassen.
...
Rang 1 §1 schaltet Ortsnamen in Fliesstext, Fussbereich und /kontakt frei. Der Code begründet ihr Fehlen weiter mit der offenen Geschäftsadresse, und WebsiteTest erzwingt kein Ortsname auf der ganzen Website. Stell den Test auf die tatsächlich gesperrten Stellen um: LocalBusiness, NAP-Aussage und Ortsseiten ohne Gate. /webdesign-dresden bleibt richtig abwesend, das Gate ist nicht erfüllt.
