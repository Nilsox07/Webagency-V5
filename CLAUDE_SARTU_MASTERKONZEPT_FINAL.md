...
### Nur kostenlose, frei weitergebbare Bausteine — eine Regel für alles

**Entscheidung:** Es kommen ausschließlich **kostenlose** Komponenten und Bibliotheken zum Einsatz,
und zwar nur solche, deren Lizenz **kommerzielle Nutzung, Veränderung und Weitergabe** erlaubt —
MIT, Apache-2.0, ISC, BSD, CC0, bei Schriften OFL.

**Warum die Weitergabe mitzählt, auch bei SARTUs eigener Seite:** Aus denselben Bausteinen entsteht
der **SARTU-Starter**, aus dem Kundenseiten erzeugt werden — ein weitergegebener Bausatz. Und jeder
Kunde bekommt seine Seite bei Vertragsende als **Export** und muss sie weiterbetreiben dürfen. Ein
Baustein ohne Weitergaberecht wäre eine Zeitbombe im Kundenprojekt.

**Damit entfällt eine Unterscheidung, die sonst nötig gewesen wäre.** Gekaufte Sammlungen erlauben
regelmäßig Kundenprojekte, verbieten aber ausdrücklich, daraus einen Bausatz zu machen — die
Tailwind-Plus-Lizenz nennt genau das. Weil SARTU nur kostenlose, permissiv lizenzierte Quellen
einsetzt, stellt sich die Frage gar nicht erst. *(Zur Klarstellung: Tailwind CSS selbst ist MIT und
unproblematisch; gemeint ist nur die gekaufte Komponentensammlung.)*

### Zwei Entstehungswege bleiben getrennt
...
| Fremde Komponenten | über den Weg oben, dokumentiert in der Herkunftsliste | **nicht direkt** — nur, was über den Starter hineinkommt |

Fremde Muster dürfen den Starter beeinflussen — der Weg führt aber immer **durch** den Starter, nie
daran vorbei. Sonst entsteht bei jedem Kunden eine eigene Lizenzlage, die niemand mehr überblickt.

Jeder Baustein trägt seine Lizenzangabe mit; die Hinweise werden mit ausgeliefert (bei MIT und
Apache Pflicht). Das ist zugleich die Grundlage für den zugesagten Export.

**Praktische Regel für die erste Zeit:** Solange der Starter entsteht, gilt für Kundenseiten im
Zweifel **selbst schreiben**. Eine unklare Lizenz im Kundenprojekt ist teurer als eine Stunde
Handarbeit — sie trifft den Kunden und damit die Betriebspauschale.
