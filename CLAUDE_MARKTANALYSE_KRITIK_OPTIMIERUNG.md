...
- **Vertriebs-/Lead-Gen-Plan** – **ergänzt** in Masterkonzept §23a (Markteintritt) und **§23b (Kanalstrategie)**: Rollenklärung „Website = Conversion, nicht Akquise" beim Start, Multiplikatoren-Netzwerk, verwaiste Kunden aussteigender Dienstleister (als **Neubau**, nicht als Übernahme fremder Systeme), Trigger-Events, Website-Schnellcheck als Doppelnutzung der QA-Technik, Branchen-Spirale, Bewertungen als Portal-Feature, 12-Monats-Sequenz. `[erledigt]`
...
### 14.2a Nachtrag 25.07.2026 — Werkzeuge, Motion, Local SEO

**Arbeitsverteilung Codex ↔ Claude Code:** Die frühere Formulierung war zu dünn. Ergänzt als **Aufgabenmatrix** (Masterkonzept §10a) mit der eisernen Regel **„pro Repository schreibt genau ein Werkzeug final"**. Kurzfassung: Codex führt alles, was Dateien, Repo, Tests, Zahlungen, Domains, QA und Deployment betrifft; Claude Code führt Strategie, Marktanalyse, SEO-/GEO-Planung, Design-Alternativen, Copy und Gegenreview. Claude Code headless bleibt **optionaler** Adapter, kein Produktionsstandard — erst nach 2–3 realen Projekten. Im Kundenvertrag wird nie ein Modell garantiert.

**Motion und Bibliotheken:** Meine Regel „keine Animationen über dezente Übergänge hinaus" war **zu streng** — eine tote Seite wirkt eher nach Baukasten. Ersetzt durch ein **Motion-Budget** (Website-Konzept §10a): ≤ 75 KB gzip JS auf der Startseite, ≤ 40 KB auf Unterseiten, **maximal zwei bewusste Markenmomente pro Seite**, `prefers-reduced-motion` verpflichtend, keine Animation über Text/CTA, kein Inhalt erst durch Scroll sichtbar. Bewertet: CSS zuerst · Lenis und GSAP+ScrollTrigger gezielt auf der Website · Motion primär im Portal · **Vanta.js, Three.js als Deko und Barba.js abgelehnt**. **Lizenzrisiko entfallen:** GSAP ist seit April 2025 vollständig kostenlos, auch kommerziell, inklusive der früher kostenpflichtigen Plugins ([Webflow](https://webflow.com/blog/gsap-becomes-free), [Lizenz](https://gsap.com/community/standard-license/), geprüft 25.07.2026).

**Local-SEO-Ortsseiten für alle Orte > 5.000 Einwohner — abgelehnt `[BELEGT]`.** Googles Doorway-Definition nennt wörtlich „*pages targeted at specific regions or cities that funnel users to one page*"; *Scaled Content Abuse* erfasst ausdrücklich auch KI-erzeugte Seiten ohne Mehrwert ([Spam Policies](https://developers.google.com/search/docs/essentials/spam-policies), [03/2024](https://developers.google.com/search/blog/2024/03/core-update-spam-policies), geprüft 25.07.2026). Verschärfend: Webdesign ist **nicht** ortsgebunden — der lokale Mehrwert steht damit unter höherem, nicht geringerem Rechtfertigungsdruck. Auch die Annahme „20 % werden indexiert, das reicht" ist falsch: 80 % ignorierte Seiten kosten Crawlbudget, verwässern interne Linkkraft und belasten das Qualitätsurteil über die ganze Domain. **Ersetzt durch** eine gestufte Struktur (Masterkonzept §16a): Region-Hub + eine starke Hauptortsseite → 10–20 priorisierte Orte durch ein Indexierungs-Gate → Branche × Region statt Kleinstadtseiten → Ausbau nur nach echten Daten. Programmatic bleibt erlaubt, aber ausschließlich mit `noindex`-Stufe und Freigabe.

**Startseite:** bleibt bewusst auf **Marke + Kategorie** optimiert, nicht auf Städte (Website-Konzept, Rollenverteilung der Seitentypen). Sie ist die stärkste URL der Domain; bei Begriffskonflikt gewinnt die Startseite, die Ortsseite wird spitzer geschnitten.

**SEO-/GEO-Taktiken** neu als Ampel-Tabelle (Masterkonzept §16b) mit klarer Trennung Whitehat / riskant / verboten. Kernaussage: Schnellere Sichtbarkeit entsteht durch **Fokus**, nicht durch Menge. Jede Taktik, die nur wirkt, solange sie unentdeckt bleibt, trifft bei einem Betreibermodell später die Kunden — und damit den wiederkehrenden Umsatz.
